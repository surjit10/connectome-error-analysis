...
- **`research_data/raw/`**: Stores the original FlyWire downloaded .csv.gz files. Folder names include the official FlyWire dataset version. Raw data is never modified. Files are read directly by the framework. The entire directory is excluded from Git.
