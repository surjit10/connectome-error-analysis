...
Load datasets into memory using Polars
Stream large files
...
Use memory-efficient loading with Polars.
...
Convert dataset tables into an immutable baseline graph representation.
...
Create graph edges
Attach attributes
Return immutable baseline graph
...
Always produce the same igraph type.
Do not compute graph statistics.
Do not modify biological metadata.
Do not apply perturbations.
Only construct the immutable baseline graph.
...
Immutable baseline graph (igraph)
Node attributes
Edge attributes
...
• Build reusable lookup/index generation
• Baseline feature extraction (computed once)
...
A validated Graph API Object enriched with reusable metadata and baseline features.
...
Compute structural features (e.g. in-degree, out-degree, total degree, PageRank, reciprocal ratio, hub neighbour count, two-hop statistics, WL rarity) exactly once.
...
Analysis (other than baseline feature extraction)
...
Operate on Temporary Analysis Graphs rather than mutable baseline graphs.
...
Do not implement algorithms here.
Maintain a stable analysis API.
Interfaces operate on Temporary Analysis Graphs.
...
Define interfaces operating on Temporary Experiment State (Working Edge Weight Array, Active Edge Mask, Temporary Edge Additions).
...
Expose interfaces only.
Do not implement specific biological error logic.
Maintain a stable error model API.
Error Framework must operate on Temporary Experiment State instead of modifying graphs.
...
Create Temporary Experiment State
Apply Error Model
Build Temporary Analysis Graph
Execute experiment loops
Destroy Temporary Graph
...
Ensure temporary analysis graphs are destroyed immediately after analysis.
...
Runtime Metadata (runtime, RAM usage, random seed, experiment metadata)
...
Include runtime, RAM usage, random seed, experiment metadata, and configuration snapshot.
...
Metadata (runtime, RAM usage, random seed, experiment metadata, configuration snapshot)
...
Only perturbation logic (operating on Temporary Experiment State)

Only analysis implementations (operating on Temporary Analysis Graphs)
...
Synapse Count Errors

Localized Reconstruction Errors

Merge Errors (requires temporary node remapping)

Split Errors (requires temporary node remapping)
```
...
Support missed synapses, false synapses, synapse count errors, and localized reconstruction errors without modifying framework code.

Merge and Split errors will later require temporary node remapping but should not affect the framework architecture.
...
* Raw datasets and the immutable baseline graph remain unchanged. Perturbations operate only on temporary experiment state. Temporary analysis graphs are created only when required and discarded immediately after analysis.
...
| 005 | FlyWireDataset | Graph API Object (igraph) | 006,007,008 |
| 006 | Graph API Object | Prepared Graph (with features) | 009 |
...
Temporary Experiment State
        │
        ├── Working Edge Weight Array
        ├── Active Edge Mask
        ├── Temporary Edge Additions
        └── Experiment Metadata
        │
        ▼
Temporary Analysis Graph
...
## Version 3 Architectural Changes

This section summarizes all architectural changes from Version 2 to Version 3.

*   **Pandas → Polars**: All DataFrames are now Polars DataFrames for memory-efficient loading and streaming.
*   **NetworkX → igraph**: The Graph API now describes an igraph-based implementation for improved scalability and performance.
*   **Immutable Baseline Graph**: The baseline graph is immutable and represents the biological connectome. It must never be modified. `graph.copy()` is completely removed.
*   **Temporary Experiment State**: Experiments no longer create graph copies. They create a Temporary Experiment State containing a Working Edge Weight Array, Active Edge Mask, Optional Temporary Edge Additions, and Experiment Metadata.
*   **Temporary Analysis Graph**: Whenever graph topology changes are required for structural updates, a Temporary Analysis Graph is constructed from the experiment state and destroyed immediately after analysis.
*   **Baseline Feature Extraction**: Structural features (in-degree, PageRank, etc.) are computed exactly once during preprocessing and become reusable baseline feature arrays.
*   **Separation of Biology and Perturbation**: Clear separation enforces Biology -> Vulnerability Model -> Probability Model -> Perturbation Engine. No stage should perform more than one responsibility.
*   **Unified Support for Multiple Reconstruction Error Models**: The architecture seamlessly supports missed synapses, false synapses, synapse count errors, and localized reconstruction errors without modifying framework code.
*   **Improved Scalability and Memory Efficiency**: The shift to Polars, igraph, immutable baseline graphs, and temporary experiment states drastically reduces memory overhead and computation time across large datasets.
